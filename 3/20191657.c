...
int main(){
	...
	while(1){
		...
		if(strcmp(comm, "help") == 0 || strcmp(comm, "h") == 0) {
			...
			if(!strcmp(comm1, "bp")){
				if(!strlen(comm2)){
					Bpprint(); Addlinkedlist(comm1);
				}
				else if(!strcmp(comm2, "clear") && !strlen(comm3)){
					strcat(comm1, " "); strcat(comm1, comm2);
					Bpclear(); Addlinkedlist(comm1);
				}
				else if(!strlen(comm3)){
					strcat(comm1, " "); strcat(comm1, comm2);
					Bp(comm2); Addlinkedlist(comm1);
				}
			}
			else if(!strcmp(comm1, "progaddr")){
				strcat(comm1, " "); strcat(comm1, comm2);
				if(Progadd(comm2)) Addlinkedlist(comm1);
			}
			else if(!strcmp(comm1, "loader") && strlen(comm2) != 0){
				char *pcom2, *pcom3, *pcom4;
				char com2[20], com3[20], com4[20];
				pcom2 = strtok(comm2, " ");
				strcpy(com2, pcom2);
				pcom3 = strtok(NULL, " ");
				if(pcom3 != NULL) {
				...
				if(strlen(com3) == 0){ // obj 1개
					strcat(comm1, " "); strcat(comm1, com2);
					if(Loader1(com2, 0) && Loader2(com2, 0)) {
						Addlinkedlist(comm1); 
						Loaderprint();
					}
				}
				else if(strlen(com4) == 0){ // obj 2개
					strcat(comm1, " "); strcat(comm1, com2); strcat(comm1, " "); strcat(comm1, com3);
					if(Loader1(com2, 0) && Loader1(com3, 1)){
					   	if(Loader2(com2, 0) && Loader2(com3, 1)) {
							Addlinkedlist(comm1);
							Loaderprint();
						}
					}
					...
					strcat(comm1, " "); strcat(comm1, com2); 
					strcat(comm1, " "); strcat(comm1, com3);
					strcat(comm1, " "); strcat(comm1, com4);
					if(Loader1(com2, 0) && Loader1(com3, 1) && Loader1(com4, 2)){
						if(Loader2(com2, 0) && Loader2(com3, 1) && Loader2(com4, 2)){
							Addlinkedlist(comm1);
							Loaderprint();
						}
					}
			...
			else if(!strcmp(comm1, "assemble") && strlen(comm2) != 0 && strlen(comm3) == 0){
				strcat(comm1, " "); strcat(comm1, comm2);
				if(Assemble(comm2) == 1) Addlinkedlist(comm1);
			}
			else if(strcmp(comm1, "opcode") == 0 && strlen(comm2) != 0 && strlen(comm3) == 0) {
			...
			else if(strcmp(comm1, "type") == 0){
				if(strlen(comm2) == 0 || strlen(comm3) != 0 || strlen(comm4) != 0){printf("Error\n"); continue;}
				
				strcat(comm1, " "); strcat(comm1, comm2);
				if(Type(comm2) == 1) Addlinkedlist(comm1);
			} // ## project2 - type filename
...
int Dumpstart(char start[]){
	...
	for(count = 0;count < 160;startrow++){
		...
		printf("%s0  ", mem[startrow].address);
...
/* Type */
// type filename 명령어를 수행한다.
// input: char filename[], return: 0 or 1

int Type(char filename[]){
	FILE *finput = fopen(filename, "r"); char ch;
	if(!finput){printf("Error\n"); return 0;}
